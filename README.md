...
Bu proje, **n8n** üzerinde çalışan ve **Google Gemini AI** modelini kullanarak PDF belgeleriyle etkileşime girmeyi sağlayan gelişmiş bir otomasyon akışıdır. Kullanıcılar PDF yükleyebilir, içerik hakkında soru sorabilir ve doküman içeriğinden oluşturulan quizler ile kendilerini test edebilirler.
...
- **Yapay Zeka Sohbeti:** Gemini AI, doküman içeriğine dayanarak kullanıcı sorularını yanıtlar.
...
- **Google Gemini (PaLM API):** Yapay zeka modeli (LLM)
...
2. **Google Gemini API** anahtarınızı `Google Gemini Chat Model` düğümüne ekleyin.
